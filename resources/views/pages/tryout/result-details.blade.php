...
@endforeach
          ...
          <div class="mt-4">
            <a href="{{ route('tryout-result', ['exam' => $exam->id, 'id' => $result->id]) }}" class="btn btn-primary w-100">
              Kembali ke Hasil
            </a>
          </div>
        </div>
    ...
    <div class="col-md-9" data-aos="fade-left" data-aos-duration="1000">
     ...
     @if(isset($question))
        @php
          $questionsBySub = $exam->questions->where('sub_category_id', $question->sub_category_id)->values();
          $questionNumberInSub = $questionsBySub->search(fn($q) => $q->id == $question->id) + 1;
        @endphp

        <h5>Pembahasan Soal No. {{ $questionNumberInSub }}</h5>
      @else
...
@endphp
  ...
  <div class="ms-3">
    ...
    {{ $difficultyCategory }}
          ...
          @php
          $currentSub   = $question->sub_category_id;
          $subQuestions = $exam->questions
              ->where('sub_category_id', $currentSub)
              ->values();

          // pakai Closure, bukan fn()
          $currentIndex = $subQuestions->search(function($item) use ($question) {
      ...
      @endphp
          ...
          @if($prevIndex >= 0)
              <a href="{{ route('tryout.review', [
                      'exam'        => $exam->id,
                      'subCategory' => $currentSub,
                      'question'    => $subQuestions[$prevIndex]->id
                  ]) }}"
          ...
          @if($nextIndex < $subQuestions->count())
              <a href="{{ route('tryout.review', [
                      'exam'        => $exam->id,
                      'subCategory' => $currentSub,
                      'question'    => $subQuestions[$nextIndex]->id
                  ]) }}"
                class="btn btn-outline-primary">
