...
@endif
    ...
    @if($exam->questions->count() > 0)
        <!-- Filter berdasarkan mata pelajaran, status, dan pembuat -->
        <div class="row mb-3">
            <div class="col-md-4">
                ...
                <select id="subCategoryFilter" class="form-control">
                    ...
                    @foreach($exam->questions->groupBy('subCategory.name') as $subCategoryName => $questionsGroup)
                        {{-- gunakan lowercase sebagai value untuk konsisten perbandingan --}}
                        <option value="{{ strtolower($subCategoryName) }}">
                            {{ $subCategoryName }} ({{ $questionsGroup->count() }})
                        </option>
                    @endforeach
                </select>
            </div>
            {{-- Jika ingin menambahkan filter difficulty, bisa dibuka lagi --}}
            {{-- <div class="col-md-4">
                    ...
                    @foreach($exam->questions as $index => $question)
                        ...
                        <tr class="question-row" 
                           ...
                           <td>{{ $loop->iteration }}</td>
                            ...
                            <td>
                                @if(strtolower($question->status ?? '') == 'diterima')
                                    <span class="badge badge-success">Diterima</span>
                                @elseif(strtolower($question->status ?? '') == 'ditolak')
                                    <span class="badge badge-danger">Ditolak</span>
                                @elseif(strtolower($question->status ?? '') == 'ditinjau')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterQuestions() {
        ...
        questionRows.forEach(row => {
            const rowSubCategory = row.dataset.subcategory; // sudah lowercase
            const rowStatus = row.dataset.status; // sudah lowercase
