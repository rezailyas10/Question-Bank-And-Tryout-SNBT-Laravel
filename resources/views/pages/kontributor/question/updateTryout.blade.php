...
<div class="form-group">
    ...
    <textarea name="question_text" id="question_text" class="form-control tiny-editor" readonly>{!! $questionTextFixed !!}</textarea>
        ...
        <div class="form-group">
            <label for="lesson">Materi atau Subbab Topik</label>
            <input type="text" name="lesson" id="lesson" class="form-control" value="{{ old('lesson', $question->lesson) }}" disabled required>
        </div>
...
@if (Auth::user()->roles === 'VALIDATOR' || (auth()->user()->roles === 'KONTRIBUTOR' && auth()->user()->is_validator == 1))
        {{-- VALIDATOR hanya ubah status --}}
        <div class="form-group">
            <label for="status">Status Pertanyaan</label>
