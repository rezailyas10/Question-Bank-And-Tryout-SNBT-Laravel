...
@endphp
    ...
    @endphp --}}
     ...
     if (auth()->user()->roles === 'KONTRIBUTOR' && auth()->user()->is_validator == 1) {
        ...
        $userQuestions = \App\Models\Question::where('exam_id', $examId)
    ->where(function($q) use ($user) {
        $q->where('sub_category_id', $user->sub_category_id)
...
{{-- @php
     if (auth()->user()->roles === 'KONTRIBUTOR' && auth()->user()->is_validator == 1) {
        // Kontributor Validator -> bisa lihat semua soal
        $userQuestions = $exam->questions->where('sub_category_id', auth()->user()->sub_category_id);
    } elseif (auth()->user()->roles === 'KONTRIBUTOR' && auth()->user()->is_validator == 0) {
        // Kontributor biasa -> hanya lihat soal miliknya sendiri
        $userQuestions = $exam->questions->where('user_id', auth()->id());
    }
                    ...
                    <select id="subCategoryFilter" class="form-control">
                        ...
                        @foreach($userQuestions->groupBy('subCategory.name') as $subCategoryName => $questions)
                            <option value="{{ $subCategoryName }}">
                                {{ $subCategoryName }} ({{ $questions->count() }})
                            </option>
                        @endforeach
                    </select>
                </div>
